...
public class QuestionServiceImpl implements QuestionService {
    ...
    private Map<QuestionDTO, QuestionEntity> dtoMapping;
    ...
    public QuestionDTO findById(final Comparable<?> primaryKey) {
        ...
        Optional<QuestionEntity> byId = questionEntityRepository.findById((Long) primaryKey);
        if (byId.isPresent()) {
            for (Map.Entry<QuestionDTO, QuestionEntity> entry : entries) {
    ...
    public QuestionDTO create(final QuestionDTO questionDTO) {
        ...
        QuestionDTO newQuestionDTO = DTOFactory.getQuestionDTO(savedQuestionEntity, surveyService.findById(questionEntity.getSurveyEntity().getId()).getPrimaryKey());
